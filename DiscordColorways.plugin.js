...
function generateCss(primaryColor, secondaryColor, tertiaryColor, accentColor, tintedText, discordSaturation) {
	return `/*Automatically Generated - Colorway Creator V${plugin.creatorVersion}*/
:root:root {
				--brand-100-hsl: ${HexToHSL("#" + accentColor)[0]} calc(var(--saturation-factor, 1)*${discordSaturation ? Math.round(HexToHSL("#" + accentColor)[1] / 100 * (100 + BrandSatDiffs[100]) * 10) / 10 : HexToHSL("#" + accentColor)[1]}%) ${Math.max(Math.round((HexToHSL("#" + accentColor)[2] + BrandLightDiffs[100]) * 10) / 10, 0)};
				--brand-130-hsl: ${HexToHSL("#" + accentColor)[0]} calc(var(--saturation-factor, 1)*${discordSaturation ? Math.round(HexToHSL("#" + accentColor)[1] / 100 * (100 + BrandSatDiffs[130]) * 10) / 10 : HexToHSL("#" + accentColor)[1]}%) ${Math.max(Math.round((HexToHSL("#" + accentColor)[2] + BrandLightDiffs[130]) * 10) / 10, 0)}%;
				--brand-160-hsl: ${HexToHSL("#" + accentColor)[0]} calc(var(--saturation-factor, 1)*${discordSaturation ? Math.round(HexToHSL("#" + accentColor)[1] / 100 * (100 + BrandSatDiffs[160]) * 10) / 10 : HexToHSL("#" + accentColor)[1]}%) ${Math.max(Math.round((HexToHSL("#" + accentColor)[2] + BrandLightDiffs[160]) * 10) / 10, 0)}%;
...
function AutoColorwaySelector({ modalProps, onChange }) {
	...
	return BdApi.React.createElement(Modals.ModalRoot, { ...modalProps }, BdApi.React.createElement(Modals.ModalHeader, null, BdApi.React.createElement(Text, { variant: "heading-lg/semibold", tag: "h1" }, "Auto Preset Settings")), BdApi.React.createElement(Modals.ModalContent, null, BdApi.React.createElement("div", { className: "dc-info-card", style: { marginTop: "1em" } }, BdApi.React.createElement("strong", null, "About the Auto Colorway"), BdApi.React.createElement("span", null, "The auto colorway allows you to use your system's accent color in combination with a selection of presets that will fully utilize it.")), BdApi.React.createElement("div", { style: { marginBottom: "20px" } }, BdApi.React.createElement(Forms.FormTitle, null, "Presets:"), Object.values(getAutoPresets()).map((autoPreset) => {
...
function SelectorHeader({ children, isSettings }) {
	if (!isSettings) {
		return BdApi.React.createElement(Modals.ModalHeader, { className: "colorwaySelectorModal-header" }, children);
	} else {
...
function Selector({
	...
	const cached_loadUI = React.useCallback(loadUI, [setColorways]);
	...
	function ReloadPopout(onClose) {
		return BdApi.React.createElement(Menu.Menu, { navId: "dc-reload-menu", onClose }, BdApi.React.createElement(
			Menu.MenuItem,
			{
				id: "dc-force-reload",
				label: "Force Reload",
	...
	return BdApi.React.createElement(SelectorContainer, { modalProps, isSettings }, BdApi.React.createElement(SelectorHeader, { isSettings }, BdApi.React.createElement(
		...
		"div",
		{
			...
			onClick: () => {
				if (currentColorway === "Auto") {
					...
					if (!betterdiscord.Data.load("settings").activeAutoPreset) {
						Modals.openModal((props) => BdApi.React.createElement(AutoColorwaySelector, { modalProps: props, onChange: (autoPresetId) => {
							...
							betterdiscord.Data.save("settings", { ...betterdiscord.Data.load("settings"), activeColorway: demandedColorway, activeColorwayID: "Auto" });
							ColorwayCSS.set(demandedColorway);
							setCurrentColorway("Auto");
						} }));
					} else {
						const demandedColorway = getAutoPresets(colorToHex(getComputedStyle(document.body).getPropertyValue("--os-accent-color")))[betterdiscord.Data.load("settings").activeAutoPreset].preset();
						betterdiscord.Data.save("settings", { ...betterdiscord.Data.load("settings"), activeColorway: demandedColorway, activeColorwayID: "Auto" });
						ColorwayCSS.set(demandedColorway);
						setCurrentColorway("Auto");
					}
		...
		currentColorway === "Auto" && BdApi.React.createElement(SelectionCircle, null)
	)) : BdApi.React.createElement(BdApi.React.Fragment, null), visibleColorwayArray.length === 0 && !getComputedStyle(document.body).getPropertyValue("--os-accent-color") ? BdApi.React.createElement(
		Forms.FormTitle,
		{
			style: {
				marginBottom: 0,
		...
		"No colorways..."
	) : BdApi.React.createElement(BdApi.React.Fragment, null), ["all", "official", "3rdparty", "custom"].includes(visibility) && visibleColorwayArray.map((color, ind) => {
					...
					if (currentColorway === color.name) {
						...
						if (betterdiscord.Data.load("settings").onDemandWays) {
							const demandedColorway = !color.isGradient ? generateCss(
								...
								colorToHex(betterdiscord.Data.load("settings").onDemandWaysOsAccentColor ? getComputedStyle(document.body).getPropertyValue("--os-accent-color") : color.accent),
								betterdiscord.Data.load("settings").onDemandWaysTintedText,
								betterdiscord.Data.load("settings").onDemandWaysDiscordSaturation
							) : gradientBase(colorToHex(betterdiscord.Data.load("settings").onDemandWaysOsAccentColor ? getComputedStyle(document.body).getPropertyValue("--os-accent-color") : color.accent), betterdiscord.Data.load("settings").onDemandWaysDiscordSaturation) + `:root:root {--custom-theme-background: linear-gradient(${color.linearGradient})}`;
							betterdiscord.Data.save("settings", { ...betterdiscord.Data.load("settings"), activeColorway: demandedColorway });
							ColorwayCSS.set(demandedColorway);
						} else {
...
function OnDemandPage() {
	const [onDemand, setOnDemand] = React.useState(betterdiscord.Data.load("settings").onDemandWays);
	const [onDemandTinted, setOnDemandTinted] = React.useState(betterdiscord.Data.load("settings").onDemandWaysTintedText);
	const [onDemandDiscordSat, setOnDemandDiscordSat] = React.useState(betterdiscord.Data.load("settings").onDemandWaysDiscordSaturation);
...
const SettingsSection = [
	{
		...
		element: () => BdApi.React.createElement(Forms.FormTitle, { style: {
			marginBottom: 0,
			padding: "6px 10px",
			color: "var(--channels-default)",
			display: "flex",
			justifyContent: "space-between"
		} }, "Discord Colorways", BdApi.React.createElement(Forms.FormTitle, { style: {
			marginBottom: 0,
			color: "var(--channels-default)",
			marginLeft: "auto"
		} }, "v", plugin.version))
