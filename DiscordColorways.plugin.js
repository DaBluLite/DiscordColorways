...
// /home/dablulite/Discord-Colorways-BD/common.ts
function customPatcher(defaultPatcher) {
	return {
		before: (object, method, callback, options) => {
			const original = object?.[method];
			if (!(original instanceof Function)) {
				throw TypeError(`patch target ${original} is not a function`);
			}
			const cancel = defaultPatcher.before(object, method, options.once ? (...args) => {
		...
		after: (object, method, callback, options) => {
			const original = object?.[method];
			if (!(original instanceof Function)) {
				throw TypeError(`patch target ${original} is not a function`);
			}
			const cancel = defaultPatcher.after(object, method, options.once ? (...args) => {
				const result = callback(cancel, original, ...args);
				cancel();
				return result;
			} : (...args) => callback(cancel, original, ...args));
		...
		instead: (object, method, callback, options) => {
			const original = object?.[method];
			if (!(original instanceof Function)) {
				throw TypeError(`patch target ${original} is not a function`);
			}
			const cancel = defaultPatcher.instead(object, method, options.once ? (...args) => {
				const result = callback(cancel, original, ...args);
				cancel();
				return result;
			} : (...args) => callback(cancel, original, ...args));
...
function getWebpack(defaultWebpack, defaultFilters) {
	return {
		...defaultWebpack,
		find: (filter, { resolve = true, entries = false } = {}) => defaultWebpack.getModule(filter, {
			defaultExport: resolve,
			searchExports: entries
		}),
		waitForStore: (storeName, callback) => {
			defaultWebpack.waitForModule(defaultFilters.byStoreName(storeName)).then((e) => callback(e));
		}
	};
}
...
const tooltip = (webpack) => webpack.getByKeys("FormItem").Tooltip;
...
const getGuildStyles = (webpack, filters) => webpack.find(filters.byKeys("guilds", "base"));
const getTreeStyles = (webpack, filters) => webpack.find(filters.byKeys("tree", "scroller"));
const ColorwayCSS = {
...
function waitForStore(storeName, callback) {
	Webpack.waitForModule(Filters.byStoreName(storeName)).then((e) => callback(e));
}
...
// common.tsx
const Webpack = getWebpack(betterdiscord.Webpack, betterdiscord.Webpack.Filters);
const Filters = getFilters(Webpack.Filters);
const CustomPatcher = customPatcher(betterdiscord.Patcher);
const Tooltip = tooltip(Webpack);
const ScrollerThin = Webpack.getByKeys("FormItem", "Button").ScrollerThin;
const Button = Webpack.getByKeys("FormItem", "Button").Button;
const Select = Webpack.getByKeys("FormItem", "Button").Select;
const TextInput = Webpack.getByKeys("FormItem", "Button").TextInput;
const Popout = Webpack.getByKeys("FormItem", "Button").Popout;
const Switch = Webpack.getByKeys("FormItem", "Button").FormSwitch;
const Card = Webpack.getByKeys("FormItem", "Button").Card;
let Clipboard;
const Text = Webpack.getByKeys("FormItem", "Button").Text;
const Forms = {
	FormTitle: Webpack.getByKeys("FormTitle").FormTitle,
	FormText: Webpack.getByKeys("FormText").FormText,
	FormDivider: Webpack.getByKeys("FormDivider").FormDivider,
	FormSection: Webpack.getByKeys("FormSection").FormSection
};
const SettingsRouter = Webpack.getByKeys("open", "init");
const Menu = {
...
const UserProfileActions = proxyLazy(() => Webpack.getByKeys("openUserProfileModal", "closeUserProfileModal"));
const UserUtils = proxyLazy(() => Webpack.getByKeys("getUser", "fetchCurrentUser"));
const Modals = {
...
const FluxDispatcher = Webpack.getModule((m) => m.dispatch && m.subscribe);
...
Webpack.waitForModule(Filters.byKeys("SUPPORTS_COPY", "copy")).then((e) => Clipboard = e);
...
const ColorVarItems = colorVariables.map((colorVariable) => {
	return {
...
function ColorPickerModal({ modalProps }) {
	const [colorVarItems, setColorVarItems] = React.useState(ColorVarItems);
	const [collapsedSettings, setCollapsedSettings] = React.useState(true);
	let results;
	function searchToolboxItems(e) {
		results = [];
		ColorVarItems.find((ToolboxItem) => {
	...
	)), BdApi.React.createElement(ScrollerThin, { style: { color: "var(--text-normal)" }, orientation: "vertical", className: collapsedSettings ? " colorwaysColorpicker-collapsed" : "", paddingFix: true }, colorVarItems.map((toolboxItem) => {
		return BdApi.React.createElement(
			"div",
			{
				id: "colorways-colorstealer-item_" + toolboxItem.id,
				className: "colorwaysCreator-settingItm colorwaysCreator-toolboxItm",
				onClick: toolboxItem.onClick,
				style: {
					"--brand-experiment": "var(--" + toolboxItem.id + ")"
				}
			},
			toolboxItem.title
		);
	})), BdApi.React.createElement(Flex, { style: { justifyContent: "space-between", marginTop: "8px" }, wrap: "wrap", className: collapsedSettings ? "" : " colorwaysColorpicker-collapsed" }, ToolboxItems.map(
		(toolboxItem, i) => BdApi.React.createElement(
			"div",
			{
				id: toolboxItem.id || `colorways-toolbox_item-${i}`,
				className: "colorwayToolbox-listItem"
			},
			BdApi.React.createElement(CopyIcon, { onClick: toolboxItem.onClick, width: 20, height: 20, className: "colorwayToolbox-listItemSVG" }),
			BdApi.React.createElement("span", { className: "colorwaysToolbox-label" }, toolboxItem.title)
		)
	)));
}

// components/ConflictingColorsModal.tsx
...
function ThemePreviewCategory({
	...
	return BdApi.React.createElement("div", { className: `${collapsed ? "colorwaysPreview colorwaysPreview-collapsed" : "colorwaysPreview"} ${className}` }, BdApi.React.createElement(
		"div",
		{
...
function ThemePreview({
	...
	return BdApi.React.createElement(
		...
		BdApi.React.createElement("style", null, previewCSS),
		BdApi.React.createElement(
			"div",
			{
			...
			BdApi.React.createElement("div", { className: "colorwaysPreview-body" }, BdApi.React.createElement("div", { className: "colorwayPreview-guilds" }, BdApi.React.createElement("div", { className: "colorwayPreview-guild" }, BdApi.React.createElement(
				...
				isFullscreen ? BdApi.React.createElement(CloseIcon, null) : BdApi.React.createElement(
					"svg",
					{
						"aria-hidden": "true",
						role: "img",
			...
			))), BdApi.React.createElement("div", { className: "colorwayPreview-channels", style: { backgroundColor: secondary } }, BdApi.React.createElement(
				"div",
				{
			...
			)), BdApi.React.createElement("div", { className: "colorwayPreview-chat", style: { backgroundColor: primary } }, BdApi.React.createElement(
				"div",
				{
...
function CreatorModal({
	...
	React.useEffect(() => {
		...
		if (parsedID) {
			...
			if (!parsedID) {
				...
				throw new Error("Invalid Colorway ID");
			} else if (!allEqual(hexToString(parsedID).split(",").map((e) => e.match("#").length)) && hexToString(parsedID).split(",").map((e) => e.match("#").length)[0] !== 1) {
				throw new Error("Invalid Colorway ID");
			} else {
				const colorArray = hexToString(parsedID).split(",");
	...
	), BdApi.React.createElement(Forms.FormTitle, { style: { marginBottom: 0 } }, "Colors:"), BdApi.React.createElement("div", { className: "colorwayCreator-colorPreviews" }, presetColorArray.includes("primary") && BdApi.React.createElement(
		ColorPicker,
		{
			label: BdApi.React.createElement(Text, { className: "colorwaysPicker-colorLabel" }, "Primary"),
	...
	), presetColorArray.includes("secondary") && BdApi.React.createElement(
		ColorPicker,
		{
			label: BdApi.React.createElement(Text, { className: "colorwaysPicker-colorLabel" }, "Secondary"),
	...
	), presetColorArray.includes("tertiary") && BdApi.React.createElement(
		ColorPicker,
		{
			label: BdApi.React.createElement(Text, { className: "colorwaysPicker-colorLabel" }, "Tertiary"),
	...
	), presetColorArray.includes("accent") && BdApi.React.createElement(
		ColorPicker,
		{
			label: BdApi.React.createElement(Text, { className: "colorwaysPicker-colorLabel" }, "Accent"),
	...
	)), BdApi.React.createElement("div", { className: `colorwaysCreator-settingCat${collapsedSettings ? " colorwaysCreator-settingCat-collapsed" : ""}` }, BdApi.React.createElement(
		"div",
		{
			className: "colorwaysCreator-settingItm colorwaysCreator-settingHeader",
			onClick: () => setCollapsedSettings(!collapsedSettings)
		},
		BdApi.React.createElement(Forms.FormTitle, { style: { marginBottom: 0 } }, "Settings"),
		BdApi.React.createElement("svg", { className: "expand-3Nh1P5 transition-30IQBn directionDown-2w0MZz", width: "24", height: "24", viewBox: "0 0 24 24", "aria-hidden": "true", role: "img" }, BdApi.React.createElement("path", { fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round", d: "M7 10L12 15 17 10", "aria-hidden": "true" }))
	), BdApi.React.createElement(ScrollerThin, { orientation: "vertical", className: "colorwaysCreator-settingsList", paddingFix: true }, BdApi.React.createElement("div", { className: "colorwaysCreator-settingItm", onClick: () => setTintedText(!tintedText) }, BdApi.React.createElement(Text, { variant: "eyebrow", tag: "h5" }, "Use colored text"), BdApi.React.createElement(Switch, { value: tintedText, onChange: setTintedText, hideBorder: true, style: { marginBottom: 0 } })), BdApi.React.createElement("div", { className: "colorwaysCreator-settingItm", onClick: () => setDiscordSaturation(!discordSaturation) }, BdApi.React.createElement(Text, { variant: "eyebrow", tag: "h5" }, "Use Discord's saturation"), BdApi.React.createElement(Switch, { value: discordSaturation, onChange: setDiscordSaturation, hideBorder: true, style: { marginBottom: 0 } })))), BdApi.React.createElement("div", { className: `colorwaysCreator-settingCat${collapsedPresets ? " colorwaysCreator-settingCat-collapsed" : ""}` }, BdApi.React.createElement(
		"div",
		{
			className: "colorwaysCreator-settingItm colorwaysCreator-settingHeader",
			onClick: () => setCollapsedPresets(!collapsedPresets)
		},
		BdApi.React.createElement(Forms.FormTitle, { style: { marginBottom: 0 } }, "Presets"),
		BdApi.React.createElement("svg", { className: "expand-3Nh1P5 transition-30IQBn directionDown-2w0MZz", width: "24", height: "24", viewBox: "0 0 24 24", "aria-hidden": "true", role: "img" }, BdApi.React.createElement("path", { fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round", d: "M7 10L12 15 17 10", "aria-hidden": "true" }))
	), BdApi.React.createElement(ScrollerThin, { orientation: "vertical", className: "colorwaysCreator-settingsList" }, BdApi.React.createElement("div", { className: "colorwaysCreator-settingItm colorwaysCreator-preset", onClick: () => {
		setPreset("default");
		setPresetColorArray(["primary", "secondary", "tertiary", "accent"]);
	} }, BdApi.React.createElement("svg", { "aria-hidden": "true", role: "img", width: "24", height: "24", viewBox: "0 0 24 24" }, BdApi.React.createElement("path", { "fill-rule": "evenodd", "clip-rule": "evenodd", d: "M12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20ZM12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z", fill: "currentColor" }), preset === "default" && BdApi.React.createElement("circle", { cx: "12", cy: "12", r: "5", className: "radioIconForeground-3wH3aU", fill: "currentColor" })), BdApi.React.createElement(Text, { variant: "eyebrow", tag: "h5" }, "Default")), Object.values(getPreset()).map((pre) => {
	...
	}))), BdApi.React.createElement(ThemePreviewCategory, { isCollapsed: false, accent: "#" + accentColor, primary: "#" + primaryColor, secondary: "#" + secondaryColor, tertiary: "#" + tertiaryColor })), BdApi.React.createElement(Modals.ModalFooter, null, BdApi.React.createElement(
		Button,
		{
			style: { marginLeft: 8 },
			color: Button.Colors.BRAND,
					...
					return BdApi.React.createElement(Modals.ModalRoot, { ...props, className: "colorwaysCreator-noMinHeight" }, BdApi.React.createElement(Modals.ModalContent, { className: "colorwaysCreator-noHeader colorwaysCreator-noMinHeight" }, BdApi.React.createElement(Forms.FormTitle, null, "Colorway ID:"), BdApi.React.createElement(TextInput, { placeholder: "Enter Colorway ID", onInput: (e) => setColorwayID(e.currentTarget.value) })), BdApi.React.createElement(Modals.ModalFooter, null, BdApi.React.createElement(
						Button,
						{
							...
							onClick: () => {
								if (!colorwayID2) {
									throw new Error("Please enter a Colorway ID");
								} else if (!hexToString(colorwayID2).includes(",")) {
									throw new Error("Invalid Colorway ID");
								} else {
									const setColor = [
										setAccentColor,
									...
									hexToString(colorwayID2).split(/,#/).forEach((color, i) => {
										var colorType = "hex";
...
function ColorwayInfoModal({
	...
	})), BdApi.React.createElement("div", { className: "colorwayInfo-row colorwayInfo-author" }, BdApi.React.createElement(Forms.FormTitle, { style: { marginBottom: 0 } }, "Author:"), BdApi.React.createElement(
		Button,
		{
			color: Button.Colors.PRIMARY,
			size: Button.Sizes.MEDIUM,
	...
	)), BdApi.React.createElement("div", { className: "colorwayInfo-row colorwayInfo-css" }, BdApi.React.createElement(Forms.FormTitle, { style: { marginBottom: 0 } }, "CSS:"), BdApi.React.createElement(
		Text,
		{
			variant: "code",
			selectable: true,
	...
	))), BdApi.React.createElement(Modals.ModalFooter, null, discrimProps && BdApi.React.createElement(
		Button,
		{
			style: { marginLeft: 8 },
			...
			onClick: async () => {
				...
				betterdiscord.Data.load("custom_colorways").map((color, i) => {
					if (betterdiscord.Data.load("custom_colorways").length > 0) {
						...
						if (betterdiscord.Data.load("settings").actveColorwayID === colorwayProps.name) {
				...
				betterdiscord.Data.load("custom_colorways").map((color, i) => {
					if (betterdiscord.Data.load("custom_colorways").length > 0) {
						...
						if (betterdiscord.Data.load("settings").activeColorwayID === colorwayProps.name) {
							betterdiscord.Data.save("settings", { ...betterdiscord.Data.load("settings"), activeColorway: color["dc-import"], activeColorwayID: color.name });
...
function SelectorModal({
	...
	return BdApi.React.createElement(Modals.ModalRoot, { ...modalProps, className: "colorwaySelectorModal" }, BdApi.React.createElement(Modals.ModalHeader, null, BdApi.React.createElement(Select, { className: "colorwaySelector-pill colorwaySelector-pill_select", options: [
		{
			value: "all",
			label: "All"
		},
	...
	], select: (value) => setVisibility(value), isSelected: (value) => visibility === value, serialize: String }), BdApi.React.createElement(
		TextInput,
		{
	...
	}), BdApi.React.createElement(Tooltip, { text: "Open Settings" }, ({ onMouseEnter, onMouseLeave }) => BdApi.React.createElement(
		Button,
		{
			innerClassName: "colorwaysSettings-iconButtonInner",
			size: Button.Sizes.ICON,
		...
		Button,
		{
			...
			onMouseEnter,
			onMouseLeave,
			onClick: () => Modals.openModal((props) => BdApi.React.createElement(ColorPickerModal, { modalProps: props }))
		},
		BdApi.React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", width: "20", height: "20", style: { padding: "6px" }, fill: "currentColor", viewBox: "0 0 16 16" }, BdApi.React.createElement("path", { d: "M12.433 10.07C14.133 10.585 16 11.15 16 8a8 8 0 1 0-8 8c1.996 0 1.826-1.504 1.649-3.08-.124-1.101-.252-2.237.351-2.92.465-.527 1.42-.237 2.433.07zM8 5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm4.5 3a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zM5 6.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm.5 6.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3z" }))
	)), BdApi.React.createElement(Tooltip, { text: "Close" }, ({ onMouseEnter, onMouseLeave }) => BdApi.React.createElement(
		Button,
		{
			innerClassName: "colorwaysSettings-iconButtonInner",
			size: Button.Sizes.ICON,
		...
		BdApi.React.createElement(CloseIcon, { style: { padding: "6px" }, width: 20, height: 20 })
	))), BdApi.React.createElement(Modals.ModalContent, { className: "colorwaySelectorModalContent" }, BdApi.React.createElement("div", { className: "colorwaysLoader-barContainer" }, BdApi.React.createElement("div", { className: "colorwaysLoader-bar", style: { height: loaderHeight } })), BdApi.React.createElement(ScrollerThin, { style: { maxHeight: "450px" }, className: "ColorwaySelectorWrapper" }, visibleColorwayArray.length === 0 && BdApi.React.createElement(
		Forms.FormTitle,
		{
			style: {
				marginBottom: 0,
		...
		return BdApi.React.createElement(Tooltip, { text: color.name }, ({ onMouseEnter, onMouseLeave }) => BdApi.React.createElement(
			...
			BdApi.React.createElement(
				...
				BdApi.React.createElement("div", { className: "colorwayInfoIcon" }, BdApi.React.createElement(
					"svg",
					{
						xmlns: "http://www.w3.org/2000/svg",
						width: "16",
			...
			BdApi.React.createElement("div", { className: "colorwayCheckIconContainer" }, BdApi.React.createElement("div", { className: "colorwayCheckIcon" }, BdApi.React.createElement(
				"svg",
				{
					xmlns: "http://www.w3.org/2000/svg",
					width: "20",
				...
				BdApi.React.createElement("circle", { r: "8", cx: "12", cy: "12", fill: "var(--white-500)" }),
				BdApi.React.createElement("g", { fill: "none", "fill-rule": "evenodd" }, BdApi.React.createElement("path", { fill: "var(--brand-500)", d: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" }))
			))),
			BdApi.React.createElement(
				"div",
				{
				...
				colors.map((colorItm) => BdApi.React.createElement(
					"div",
					{
...
// components/ColorwaysButton.tsx
function ColorwaysButton({
	...
	async function setButtonVisibility() {
		setVisibility(betterdiscord.Data.load("settings").showInGuildBar);
	}
	const cached_setButtonVisibility = React.useCallback(setButtonVisibility, []);
	React.useEffect(() => {
	...
	if (!isThin) {
		return BdApi.React.createElement(
			Tooltip,
			{
				text: BdApi.React.createElement(BdApi.React.Fragment, null, BdApi.React.createElement("span", null, "Colorways"), BdApi.React.createElement(Text, { variant: "text-xs/normal", style: { color: "var(--text-muted)", fontWeight: 500 } }, "Active Colorway: " + activeColorway)),
				position: "right",
				tooltipContentClassName: listItemTooltipClass
			},
			({ onMouseEnter, onMouseLeave, onClick }) => {
				return BdApi.React.createElement(BdApi.React.Fragment, null, visibility && BdApi.React.createElement("div", { className: listItemClass }, BdApi.React.createElement(
					"div",
					{
						...
						onClick: () => {
							onClick();
							Modals.openModal((props) => BdApi.React.createElement(SelectorModal, { modalProps: props }));
						}
					},
		...
		return BdApi.React.createElement(
			...
			({ onMouseEnter, onMouseLeave, onClick }) => {
				return BdApi.React.createElement(BdApi.React.Fragment, null, visibility && BdApi.React.createElement("div", { className: listItemClass }, BdApi.React.createElement(
					...
					BdApi.React.createElement(Text, { variant: "text-xs/normal", style: { color: "var(--header-primary)", fontWeight: 700, fontSize: 9 } }, "Colorways")
				)));
			}
		);
	}
...
function SelectorPage({
	...
	return BdApi.React.createElement(SettingsTab, { title: "Colors" }, BdApi.React.createElement("div", { className: "colorwaysSettingsSelector-wrapper" }, BdApi.React.createElement(Flex, { style: { gap: "0" } }, BdApi.React.createElement(Select, { className: "colorwaySelector-pill colorwaySelector-pill_select", options: [
		{
			value: "all",
			label: "All"
		},
		...
		setVisibility(value);
	}, isSelected: (value) => visibility === value, serialize: String }), BdApi.React.createElement(
		TextInput,
		{
			inputClassName: "colorwaySelector-searchInput",
			className: "colorwaySelector-search",
	...
	))), BdApi.React.createElement("div", { className: "colorwaysLoader-barContainer" }, BdApi.React.createElement("div", { className: "colorwaysLoader-bar", style: { height: loaderHeight } })), BdApi.React.createElement("div", { className: "ColorwaySelectorWrapper" }, visibleColorwayArray.length === 0 && BdApi.React.createElement(
		Forms.FormTitle,
		{
			style: {
				marginBottom: 0,
...
// components/SettingsTabs/SettingsPage.tsx
function SettingsPage() {
	const [colorways, setColorways] = React.useState([]);
	const [customColorways, setCustomColorways] = React.useState(betterdiscord.Data.load("custom_colorways"));
	...
	return BdApi.React.createElement(SettingsTab, { title: "Settings" }, BdApi.React.createElement("div", { className: "colorwaysSettingsPage-wrapper" }, BdApi.React.createElement(Flex, { style: { gap: "0", marginBottom: "8px" } }, BdApi.React.createElement(Forms.FormTitle, { tag: "h5", style: { width: "100%", marginBottom: "0", lineHeight: "32px" } }, "Sources"), BdApi.React.createElement(
		Button,
		{
			...
			onClick: () => {
				Modals.openModal((props) => {
					var colorwaySource = "";
					return BdApi.React.createElement(Modals.ModalRoot, { ...props }, BdApi.React.createElement(Modals.ModalHeader, null, BdApi.React.createElement(Text, { variant: "heading-lg/semibold", tag: "h1" }, "Add a source:")), BdApi.React.createElement(Modals.ModalContent, null, BdApi.React.createElement(
						TextInput,
						{
	...
	)), BdApi.React.createElement(Divider, null), BdApi.React.createElement(Forms.FormTitle, { tag: "h5" }, "Quick Switch"), BdApi.React.createElement(
		Switch,
		{
...
// components/SettingsTabs/OnDemandPage.tsx
...
// components/SettingsTabs/ManageColorwaysPage.tsx
function ManageColorwaysPage() {
	return BdApi.React.createElement(SettingsTab, { title: "Manage Colorways" }, BdApi.React.createElement(Forms.FormSection, { title: "Import/Export" }, BdApi.React.createElement(Card, { className: "vc-settings-card vc-backup-restore-card" }, BdApi.React.createElement(Flex, { flexDirection: "column" }, BdApi.React.createElement("strong", null, "Warning"), BdApi.React.createElement("span", null, "Importing a colorways file will overwrite your current custom colorways."))), BdApi.React.createElement(Text, { variant: "text-md/normal", style: { marginBottom: "8px" } }, "You can import and export your custom colorways as a JSON file. This allows you to easily transfer them to another device/installation."), BdApi.React.createElement(Flex, null, BdApi.React.createElement(
		Button,
		{
			size: Button.Sizes.SMALL,
			onClick: async () => {
...
// index.tsx
const guildStyles = getGuildStyles(Webpack, Filters);
const treeStyles = getTreeStyles(Webpack, Filters);
const forceFullRerender = (fiber) => new Promise((resolve) => {
	const owner = findOwner(fiber);
	if (owner) {
		const { stateNode } = owner;
		CustomPatcher.instead(stateNode, "render", () => null, { once: true, silent: true });
...
betterdiscord.Data.save("custom_colorways", Object.assign(
	[],
	betterdiscord.Data.load("custom_colorways")
));
const GuildsNav = Webpack.find(Filters.bySource("guildsnav"), { entries: true });
class DiscordColorways {
	load() {
	}
	start() {
		CustomPatcher.after(GuildsNav, "type", (cancel, result, ...args) => {
			const target = queryTree(args[2], (node) => node?.props?.className?.split(" ").includes(guildStyles.guilds));
			if (!target) {
			...
			hookFunctionComponent(target, (result2) => {
				...
				const homeButtonIndex = children.findIndex((child) => Filters.bySource(".getPendingCount")(child?.type));
				const index = homeButtonIndex > -1 ? homeButtonIndex + 1 : 2;
				children.splice(index, 0, BdApi.React.createElement(ColorwaysButton, null));
			});
